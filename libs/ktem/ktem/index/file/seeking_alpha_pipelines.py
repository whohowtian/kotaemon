...
class IndexPipeline(BaseComponent):
    ...
    def extract_speakers(self, text: str) -> tuple[list[str], dict[str, str]]:
        ...
        pattern = r'(?:Company Participants|Executives|Corporate Participants)(.*?)(?:Conference Call Participants|Analysts|Other Participants)\n(.*?)\n'
    ...
    def extract_prefix(self, line: str) -> str:
    ...
    def preprocess_transcript(self, text_docs: list[Document]) -> list[Document]:
        ...
        stock_ticker = self.extract_ticker(file_name)
        
        transcript_type = 'Earnings call' if 'Earnings' in file_name else 'Others'
        fiscal_quarter = self.extract_quarter(file_name) if transcript_type == 'Earnings call' else None
        ...
        for i, (speaker, text_chunk) in enumerate(chunks):
            ...
            metadata = {
                **original_metadata,
                ...
                "transcript_name": f"{self.extract_prefix(file_content)}{file_name}",
    ...
    def stream(
        ...
        docs = self.loader.load_data(file_path, extra_info=extra_info)  # txt returns list of 1 document
...
class IndexSeekingAlphaPipeline(BaseFileIndexIndexing):
